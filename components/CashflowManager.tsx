...
const TransactionModal: React.FC<{
    ...
    const handleSave = () => {
        ...
        if (!finalContact) {
            finalContact = { id: generateUniqueId('custom-'), name: contactSearch.trim() };
        }

        const finalTransaction: CashTransaction = {
            id: formData.id || generateUniqueId('CT-'),
            type: formData.type!,
            ...
            {activeTab === 'balance' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {paymentSources.map(source => {
                        // FIX: Cast `val` to `number` as `Object.values` can return `unknown[]` with strict TS configs.
                        const totalBalance = Object.values(source.balance).reduce((acc: number, val) => acc + (val as number), 0);
                        return (
